...
# Convert the data into a 3D array for input into the MODEL
...
model %>%
   bidirectional(layer_simple_rnn(units, return_sequences = FALSE), input_shape = c(look_back + 1, ncol(train_data) - 1)) %>%
